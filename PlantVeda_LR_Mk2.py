...
def predict(input_data):
    ...
    # LR gives decimal — round and clip to valid class range
    pred_raw = model.predict(user_row)[0]
    pred_enc = int(np.round(np.clip(pred_raw, 0, len(le_target.classes_) - 1)))
